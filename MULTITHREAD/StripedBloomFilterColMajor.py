...
# worker for column-major
def _worker_contains_striped_col_major(args):
    """Test Pattern: COLUMN-MAJOR (Layer-Outer, Item-Inner Loop)"""
    ...
    for item in items:
        if type(item) is str:
            ...
            item_bytes = str(item).encode('utf-8')
        hashes.append(mmh3.hash64(item_bytes, seed=42, signed=False))

    # array of results initialized to False (np.zeros)
    results = np.zeros(len(items), dtype=bool)
    ...
    for layer_idx, stripe_buf in enumerate(stripes_per_layer):
        ...
        for item_idx, (h1, h2) in enumerate(hashes):
...
class StripedBloomFilterColMajor:
    ...
    def add_batch(self, items: list):
        ...
        while current_idx < total_items:
            ...
            item_chunks = [items_for_this_layer[i:i + chunk_size] for i in
                           range(0, len(items_for_this_layer), chunk_size)]
    ...
    def contains_batch(self, items: list) -> list[bool]:
        ...
        # calling new worker with Col-Major
        worker_results = list(self.executor.map(_worker_contains_striped_col_major, jobs))
